...
Currently Shopify only allows 500 calls per 5 minutes when using the API, if you go over that limit it will throw an exception.
...
The block will be retried up to a number times, and will sleep for an increasing number of seconds between retries. This allows for the full 5 minutes to pass before the limit is reset.
